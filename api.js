...
class Api {
  ...
  constructor(opts) {
    ...
    this._showHelpByDefault =
      "showHelpByDefault" in opts ? opts.showHelpByDefault : false;
  ...
  configure(opts) {
    ...
    if ("factories" in opts) {
      Object.keys(opts.factories).forEach((name) =>
        this.registerFactory(name, opts.factories[name])
      );
    }
  ...
  _assignHelpOpts(target, source) {
    [
      ...
      "styleMessages",
      "styleUsagePrefix",
      "styleUsagePositionals",
      "styleUsageCommandPlaceholder",
      "styleUsageArgsPlaceholder",
      "styleUsageOptionsPlaceholder",
      "styleExample",
      "styleAll",
    ].forEach((opt) => {
  ...
  addStrictModeErrors(context) {
    if (this._strictMode) {
      ...
      if (unknownOptions.length > 0) {
        context.cliMessage(
          `Unknown options: ${unknownOptions.map((u) => u.raw).join(", ")}`
        );
      }
  ...
  async commandDirectory(dir, opts) {
    ...
    let filepath;
    let mod;
    const tasks = this.fsLib.readdirSync(searchDir).map(async (fileInDir) => {
      filepath = this.pathLib.join(searchDir, fileInDir);
      if (
        opts.extensions.indexOf(this.pathLib.extname(fileInDir)) !== -1 &&
        this._modulesSeen.indexOf(filepath) === -1
      ) {
        this._modulesSeen.push(filepath);
        mod = (await import(filepath)).default;
        if (mod.flags || mod.aliases) {
          this._internalCommand(mod);
        } else if (typeof mod === "function") {
          this._internalCommand({
            aliases: this.pathLib.basename(
              fileInDir,
              this.pathLib.extname(fileInDir)
            ),
            run: mod,
          });
        }
  ...
  _internalCommand(dsl, opts) {
    ...
    opts.aliases = opts.aliases
      ? Array.from(new Set([name].concat(opts.aliases)))
      : [name];
    if (mp.length) {
  ...
  positional(dsl, opts) {
    ...
    if (Array.isArray(dsl)) {
      ...
      this.helpOpts.usagePositionals = (
        this.helpOpts.usagePositionals || []
      ).concat(dsl);
      addedToHelp = true;
      ...
      if (!opts.params) {
        ...
        opts.params = array.map((string, index) => {
          return opts.params[index]
            ? Object.assign({ flags: string }, opts.params[index])
            : string;
        });
      } else {
    ...
    const params = Array.isArray(opts.params)
      ? opts.params.slice()
      : Object.keys(opts.params).map((key) => {
          let obj = opts.params[key];
    ...
    params.forEach((param, index) => {
      ...
      if (
        ...
        (opts.paramsDescription || opts.paramsDesc)
      ) {
        param.desc = [].concat(opts.paramsDescription || opts.paramsDesc)[
          index - numSkipped
        ];
      }
      ...
      param = Object.assign(
        this.utils.inferPositionalProperties(
          positionalFlags,
          Object.keys(this._factories)
        ),
        param
      );
  ...
  _getArrayType(flags, opts, subtypeName) {
    ...
    if (subtypeName.indexOf(":") !== -1) {
      ...
      if (types[0] === "array") {
        opts.elementType = this._getArrayType(
          flags,
          opts,
          types.slice(1).join(":") || "string"
        );
  ...
  async parseAndExit(args, state) {
    ...
    if (result.output) {
      ...
      const level =
        (context.helpRequested || context.versionRequested) && result.code === 0
          ? "log"
          : "error";
  ...
  async _parse(args, state) {
    ...
    try {
      ...
      if (
        (context.helpRequested || context.helpRequestedImplicitly) &&
        !context.output
      ) {
  ...
  async parseFromContext(context) {
    ...
    if (
      ...
      this._internalCommand(Api.DEFAULT_COMMAND_INDICATOR, (argv, context) => {
        ...
        {
          api: this.newChild(Api.DEFAULT_COMMAND_INDICATOR, {
            strictMode: false,
          }),
        },
        false
    ...
    context.populateArgv(
      this.types.map((type) => type.toResult(context, shouldCoerceAndCheck))
    );
  ...
  initHelpBuffer() {
    const helpOpts = Object.assign(
      { utils: this.utils, usageName: this.name },
      this.helpOpts
    );
